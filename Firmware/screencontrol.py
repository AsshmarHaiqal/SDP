...
class PillDispenserUI:
    def __init__(self, root):
        ...
        # REDUCED LOG HEIGHT - Now only ~5% of screen (80 pixels instead of 150)
        self.log_frame = tk.Frame(root, bg="white", height=80)
        self.log_frame.pack(side="bottom", fill="x")
        self.log_frame.pack_propagate(False)

        tk.Label(self.log_frame, text="System Log:", font=("Arial", 10, "bold"),
                 bg="white").pack(anchor="w", padx=10, pady=2)

        # Smaller text widget with only 3 lines visible instead of 6
        self.log_text = tk.Text(self.log_frame, font=("Arial", 9),
                                bg="white", height=3)
        self.log_text.pack(expand=True, fill="both", padx=10, pady=2)
